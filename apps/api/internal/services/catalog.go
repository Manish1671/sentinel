...
func (r *Repository) list(ctx context.Context, filter ListFilter, cursorTime time.Time, cursorID uuid.UUID) ([]ServiceRecord, error) {
	q := `
		SELECT id, slug, name, environment::text, description, owner_user_id, health_status::text
		FROM services
		WHERE ($1 = '' OR environment::text = $1)
		  AND ($2 = '' OR health_status::text = $2)
		  AND ($3 OR (slug, id) > ((SELECT slug FROM services WHERE id = $4), $4))
		ORDER BY slug, id
		LIMIT $5
	`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rec.ID, &rec.Slug, &rec.Name, &rec.Environment, &rec.Description, &rec.OwnerUserID, &rec.HealthStatus); err != nil {
			return nil, err
		}
		out = append(out, rec)
	}
...
func (r *Repository) get(ctx context.Context, id uuid.UUID) (ServiceRecord, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		FROM services
		WHERE id = $1
	`, id).Scan(&rec.ID, &rec.Slug, &rec.Name, &rec.Environment, &rec.Description, &rec.OwnerUserID, &rec.HealthStatus)
	return rec, err
}
